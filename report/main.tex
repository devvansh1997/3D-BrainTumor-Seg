...
Figures~\ref{fig:seg1} and~\ref{fig:seg2} show axial segmentation overlays
for two representative BraTS2021 cases.
Predictions are generated by running inference with the trained MSD checkpoints
on local BraTS2021 data to illustrate cross-dataset generalisation.
The three tumor sub-regions are coloured: NCR/NET in red, ED in yellow, ET in cyan.

\begin{figure}[H]
    \centering
    \includegraphics[width=\linewidth]{figures/fig5_seg_BraTS2021_00000.png}
    \caption{Axial segmentation overlays for BraTS2021\_00000 (edema-dominant case).
             Columns left to right: T1ce image, Ground Truth, DynUNet+BoundaryLoss,
             SwinUNETR. Rows show three axial slices through the tumour centre.
             NCR/NET = red, ED = yellow, ET = cyan.}
    \label{fig:seg1}
\end{figure}

\begin{figure}[H]
    \centering
    \includegraphics[width=\linewidth]{figures/fig5_seg_BraTS2021_00002.png}
    \caption{Axial segmentation overlays for BraTS2021\_00002 (ET-dominant case).
             Same column layout as Figure~\ref{fig:seg1}.
             Both models correctly localise the tumour; some over-prediction of NCR
             (red) relative to GT is expected given cross-dataset transfer
             (models trained on MSD, evaluated on BraTS2021).}
    \label{fig:seg2}
\end{figure}
